...
**/
func setNewArticleInfoToCache(id int, article *ArticleInfo) error {
	rc := juneDao.GetRedisConn()
	defer rc.Close()
	fields := []string{
...
**/
func articleInfoMissHitsUpdate(id int) (ArticleInfo, error) {
	ali, err := QueryArticleInfoByIdInDb(id)
	if err != nil {
...
**/
func queryArticleInfoFromCache(id int) (ArticleInfo, error) {
	var fields = []string{
...
**/
func queryArticleInfoByLimitByCache(page, pageSize, total int) ([]ArticleInfo, error) {
...
* Time: 2020/9/11 23:27
**/
func QueryArticleInfoByLimit(page, pageSize int) ([]ArticleInfo, int, error) {
	...
	articleListInfos := make([]ArticleInfo, len(articleList))
	for i, article := range articleList {
		...
		articleListInfos[i] = ArticleInfo{
			Tags:       tags,
			ID:         article.ID,
			Title:      article.Title,
...
**/
func QueryArticleInfoByLimitWithTag(tagID, page, pageSize int) ([]ArticleInfo, int, error) {
	...
	articleList := make([]ArticleInfo, 0)
...
func QueryAllArticle() ([]Article, error) {
	articleList := make([]Article, 0)
	err := juneDao.GetDB().Order("create_time desc").Find(&articleList).Error
	if err != nil {
		return nil, err
	}
...
**/
func QueryArticleInfoByIdInDb(id int) (ArticleInfo, error) {
	result := ArticleInfo{}
	article := Article{}
...
* Time: 2020/9/24 10:56
**/
func QueryArticleByID(id int) (ArticleInfo, error) {
	var articleListInfo ArticleInfo
	if internal.GetSetting().Others.Redis {
		return queryArticleInfoFromCache(id)
	}
	articleListInfo, err := QueryArticleInfoByIdInDb(id)
...
func addArticleWithCache(newArticle *Article, tags []Tag) error {
	...
	// 更新 ArticleInfo
	err = setNewArticleInfoToCache(newArticle.ID, &ArticleInfo{
		ID:         newArticle.ID,
...
func NewArticle(newArticle *Article, tagIDs []int) (*Article, error) {
